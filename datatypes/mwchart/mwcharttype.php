...
class mwChartType extends eZDataType
{
    const MAX_FILESIZE_FIELD = 'data_int1';
    const MAX_FILESIZE_VARIABLE = '_mwchart_max_filesize_';
    const DATA_TYPE_STRING = "mwchart";
    ...
    */
    ...
    {
        parent::__construct( self::DATA_TYPE_STRING, ezpI18n::tr( 'extension/mw_chart/datatype', 'MW Chart', 'Datatype name' ), array( 'serialize_supported' => true, 'object_serialize_map' => array( 'data_text' => 'filename' )));
    }

    function mwChartType()
    {
        $this->eZDataType( self::DATA_TYPE_STRING, ezpI18n::tr( 'kernel/classes/datatypes', "MW Chart", 'Datatype name' ),
    ...
    */
    ...
    {
        if($http->hasPostVariable('ChartType')){
            $contentObjectAttribute->setAttribute('data_text', $http->postVariable('ChartType'));
            $contentObjectAttribute->store();
        }
    /*gets hit the first time through*/
        mwChartType::checkFileUploads();
        if ( mwChartType::canFetch( $base . "_data_enhancedbinaryfilename_" . $contentObjectAttribute->attribute( "id" ) ) )
                return false;
    ...
    */
    ...
    function objectAttributeContent( $contentObjectAttribute )
    {
        ...
        $type = $contentObjectAttribute->attribute('data_text');
        ...
        $obj = new mwChartObject($binaryFile,'Orkla Graph', $type);
        return $obj;
    }
...
*/
        $canFetchResult = mwChartType::canFetch( $base . "_data_enhancedbinaryfilename_" . $contentObjectAttribute->attribute( "id" ), $maxSize );
    ...
    */
    function unserializeContentObjectAttribute( $package, $objectAttribute, $attributeNode )
    {
        ...
        eZDebug::writeNotice( 'Copied: ' . $sourcePath . ' to: ' . $destinationPath . $basename,
                              'mwChartType::unserializeContentObjectAttribute()' );
...
eZDataType::register( mwChartType::DATA_TYPE_STRING, "mwChartType" );
